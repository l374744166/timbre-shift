...
export function SongCard(option, selectedId = '') {
  ...
  return `<article class="resource-card ${option.value === selectedId ? 'selected' : ''}" data-song-id="${escapeHtml(option.value)}">
    ...
    <div class="card-actions"><button type="button" class="secondary song-select" data-id="${escapeHtml(option.value)}">选择</button><button type="button" class="secondary song-detail" data-id="${escapeHtml(option.value)}">详情</button><button type="button" class="secondary danger song-delete" data-id="${escapeHtml(option.value)}" data-name="${title}">删除</button></div>
  </article>`;
}
