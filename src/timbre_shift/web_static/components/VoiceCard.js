...
export function VoiceCard(option, selectedId = '', mode = 'seedvc') {
  ...
  if (isRvc) {
    ...
    return `<article class="resource-card ${option.value === selectedId ? 'selected' : ''}" data-voice-id="${escapeHtml(option.value)}">
      ...
      <div class="card-actions"><button type="button" class="voice-train" data-id="${escapeHtml(option.value)}">${trainLabel}</button><button type="button" class="secondary voice-select" data-id="${escapeHtml(option.value)}">选择生成</button></div>
  ...
  return `<article class="resource-card ${option.value === selectedId ? 'selected' : ''}" data-voice-id="${escapeHtml(option.value)}">
    ...
    <div class="card-actions"><button type="button" class="secondary voice-select" data-id="${escapeHtml(option.value)}">选择参考声音</button></div>
  </article>`;
}
