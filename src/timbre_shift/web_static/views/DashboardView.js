...
import { resultFacts, resultNotices, scoreItems } from './DashboardResultDetails.js';
import { formatSeconds, rvcTrainingPanel, sampleRow, modelRow } from './DashboardTrainingPanel.js';
...
export const DashboardView = {
  ...
  mount: (options = {}) => {
    ...
    const refreshPreflight = () => renderPreflight({
      isRvc: isRvc(),
    ...
    qs('#voiceCards')?.addEventListener('click', async (event) => {
      ...
      const detailButton = event.target.closest('.voice-detail');
      if (detailButton) {
        await openVoiceDetail({
          voiceId: detailButton.dataset.id,
          mode: state.selectedEngine,
          selectVoice,
