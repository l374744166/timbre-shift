...
export const DashboardView = {
  render: () => {
    ...
    <section class="step-section"><h3>${copy.voiceTitle}</h3><p class="muted">${copy.voiceHelp}</p><div class="resource-grid" id="voiceCards">${voiceCards()}</div>
      <div class="upload-strip voice-upload-strip"><input id="voice" name="voice" type="file" accept="audio/*" multiple><input id="voiceName" name="voice_name" placeholder="${copy.voiceNamePlaceholder}"><select id="voiceSourceType" name="voice_source_type" aria-label="${copy.sourceLabel}"><option value="clean_voice">干净人声</option><option value="mixed_voice">带伴奏，自动分离人声</option></select><button class="secondary" id="saveVoiceButton" type="button">${copy.saveButton}</button><button class="secondary" id="addVoiceSampleButton" type="button">${copy.addButton}</button></div><div class="message" id="voiceSaveMessage"></div></section>
    ...
    <input type="hidden" name="mode" value="m2max_hq_30"><input type="hidden" id="voiceModel" name="voice_model_id"><input type="hidden" name="song_id" id="songIdField"><input type="hidden" name="voice_profile_id" id="voiceIdField"><input type="hidden" name="mix_style" value="natural">
